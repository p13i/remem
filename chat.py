...
def gpt3_completion(prompt, engine='text-davinci-003', temp=0.0, top_p=1.0, tokens=400, freq_pen=0.0, pres_pen=0.0, stop=['USER:', 'NINJA:']):
    max_retry = 5
    retry = 0
...
if __name__ == '__main__':
    ...
    while True:
        ...
        info = {'speaker': 'NINJA', 'time': time(), 'vector': vector, 'message': output, 'uuid': str(uuid4())}
        filename = 'log_%s_NINJA.json' % time()
        save_json('chat_logs/%s' % filename, info)
        #### print output
        print('\n\nNINJA: %s' % output)
